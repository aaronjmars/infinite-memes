  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      <div className="mt-8 text-center ">
        <Link
          href="/"
          className="inline-block bg-gradient-to-r from-cyan-500 via-blue-500 to-purple-500 text-white px-6 py-3 rounded-full hover:from-cyan-600 hover:via-blue-600 hover:to-purple-600 transition-colors"
        >
          Generate More Memes
        </Link>
      </div>
        ...
        {shuffledUrls.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {shuffledUrls.map((url, index) => (
              <div
                key={index}
                className="rounded-lg overflow-hidden cursor-pointer transition-all duration-300 ease-in-out transform hover:scale-105"
              >
